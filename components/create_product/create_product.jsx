...
const CreateProduct = () => {
    ...
    const handleAddColor = () => {
        if (!color ||  listSizeChose.length < 1 || listImage.length < 1)
            ...
            setListColorChose([...listColorChose, {color: color, size: listSizeChose, num: numProduct, image: listImage}])
            setColor({})
            setNumProduct(0)
            setListSizeChose([])
            setListImage([])
    ...
    const handleAddProduct = () => {
        if (!productName || !costProduct || !category || listColorChose.length < 1)
            ...
            openNotificationWithIcon('success', `Thêm sản phẩm ${productName} thành công`)
            const newProduct = {
                name: productName, 
                price: costProduct,
    ...
    return (
        <div className="w-[100%]">
            <div className="w-[100%] bg-gray-100 flex flex-row">
                ...
                <div className="w-[65%] p-8">   
                    ...
                    <div className='shadow-md px-8 py-10 mb-6 bg-white rounded-md'>
                        ...
                        <Input className="rounded-full px-4" type="text" placeholder="Nhập tên sản phẩm" value={productName} onChange={handleProductNameChange}/>
                        <div className='flex flex-row justify-between'>
                            <div className="w-[100%]">
                                <p className='text-sm mx-2 mb-1 mt-6'>Giá sản phẩm</p>
                                <Input className="rounded-full px-4" type="number" placeholder="Nhập giá sản phẩm" value={costProduct} onChange={handleCostProductChange}/>
                            </div>
                        </div>
                        <p className='text-sm mx-2 mb-1 mt-6'>Category</p>
                        <div className='flex flex-row justify-between gap-4'>
                            <Select onValueChange={handleSetCategory}>
                                <SelectTrigger className=" rounded-full px-4"><SelectValue placeholder="Chọn Category"/></SelectTrigger>
                                <SelectContent>
                                    <ScrollArea className="h-30 w-[100%]">
                                        <SelectGroup>{listCategory.map((item, index) => (
                                            <SelectItem key={index} value={index}>{getAllCategoryNames(item)}</SelectItem>))}
                                        </SelectGroup>
                                    </ScrollArea>
                                </SelectContent>
                            </Select>
                            <Button className='rounded-full p-4 hover:bg-gray-200 hover:text-black'>Thêm Categogy mới</Button>
                        </div>
                        <p className='text-sm mx-2 mb-1 mt-6'>Đặc điểm nổi bật</p>
                        <Textarea  className="rounded-xl px-4 " placeholder="Nhập ..." value={description} onChange={handleDescriptionChange}/>
                    ...
                    <div className='shadow-md px-8 py-10 mb-6 bg-white rounded-md'>
                        ...
                        <Select onValueChange={handleSetColor}>
                            <SelectTrigger className=" rounded-full px-4"><SelectValue placeholder="Chọn màu sắc"/></SelectTrigger>
                            <SelectContent>
                                <ScrollArea className="h-30 w-[100%]">
                                    <SelectGroup>{listColor.map((item, index) => (
                                        <SelectItem key={index} value={index}>
                                            <div className='flex flex-row items-center'>
                                                <img className="w-10 h-6 object-cover rounded-full mr-4" src={item.img} alt="color"/>
                                                <span>{item.name}</span>
                                            </div>
                                        </SelectItem>))}
                                    </SelectGroup>
                        ...
                        <Input className="rounded-full px-4" type="number" placeholder="Nhập số lượng" value={numProduct} onChange={handleNumProductChange}/>
                        <p className='text-sm mb-3 mt-6'>Kích cỡ</p>
                        <div className='flex flex-row items-center justify-between mb-8'>
                            {listSize.map((item, index) => (
                                <div key={index} className="flex items-center space-x-1">
                                    <input type="checkbox" value={item} id={item} checked={listSizeChose.includes(item)}
                        ...
                        <Button className='rounded-full p-4 hover:bg-gray-200 hover:text-black mt-8' onClick={handleAddColor}>Thêm màu {color.name}</Button>
                ...
                <div className='w-[30%] mt-[92px]'>
                    <div className='shadow-md px-8 py-10 mb-6 bg-white rounded-md'>
                        <p className='font-bold mb-4'>Trạng thái sản phẩm</p>
                            ...
                            <RadioGroup className="flex flex-row my-4 gap-6" defaultValue="Disabled" onValueChange={handleStatusChange}>
                                <div className="flex items-center space-x-1">
                                    <RadioGroupItem className={status==="Disabled" ? "border-blue-600 text-blue-600" : ""} value="Disabled" id="Disabled" />
                                    <Label htmlFor="Disabled">Disabled</Label>
                                </div>
                                <div className="flex items-center space-x-1">
                                    <RadioGroupItem className={status==="Enabled" ? "border-blue-600 text-blue-600" : ""} value="Enabled" id="Enabled" />
                                    <Label htmlFor="Enabled">Enabled</Label>
                                </div>
                            </RadioGroup>
                            <p className='text-sm mb-3 mt-6'>Visibility</p>
                            <RadioGroup className="flex flex-row my-4 gap-6" defaultValue="Not visible" onValueChange={handleVisibilityChange}>
                                <div className="flex items-center space-x-1">
                                    <RadioGroupItem className={visibility==="Not visible" ? "border-blue-600 text-blue-600" : ""} value="Not visible" id="Not visible" />
                                    <Label htmlFor="Not visible">Not visible</Label>
                                </div>
                                <div className="flex items-center space-x-1">
                                    <RadioGroupItem className={visibility==="Visible" ? "border-blue-600 text-blue-600" : ""} value="Visible" id="Visible" />
            ...
            <div className='w-[100%] bg-gray-100 '>
                <Button className='w-[95%] rounded-full p-8 mb-20 ml-8' onClick={handleAddProduct}>Hoàn tất</Button>
            </div>
        </div>
