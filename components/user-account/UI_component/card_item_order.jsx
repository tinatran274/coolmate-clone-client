        ...
        <div className="h-[100%] w-[100%] bg-gray-200 rounded-md mt-6 cursor-pointer">
            <div className='flex justify-between items-center bg-blue-700 px-6 py-3 rounded-md mt-6'>
                <div>
                    ...
                    <p className='text-xs text-white'>{detaiOrder.date}</p>
                </div>
                <p className='text-xs font-bold text-black bg-white rounded-full p-2'>{detaiOrder.status}</p>   
            </div>
            <div className='px-3 pb-1'>
                {detaiOrder.listProduct.map((product, index) => {
                return (
                    <CardItemPay
                        key={index}
                        name={product.name} 
            ...
            <div className='flex flex-row justify-end px-6 py-5'>
                <p className='font-bold text-sm'>{addDotsToNumber(detaiOrder.totalCost)}đ</p>
            </div>
